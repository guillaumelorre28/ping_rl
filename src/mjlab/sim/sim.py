...
class Simulation:
  ...
  **Important:** A captured graph holds pointers to the GPU arrays that existed
  ...
  def __init__(
    ...
    self._mj_fk_data = mujoco.MjData(model)
    mujoco.mj_forward(self._mj_model, self._mj_data)
    mujoco.mj_forward(self._mj_model, self._mj_fk_data)
    ...
    with wp.ScopedDevice(self.wp_device):
      ...
      self._wp_fk_data = mjwarp.put_data(
        self._mj_model,
        self._mj_fk_data,
        nworld=self.num_envs,
        nconmax=self.cfg.nconmax,
    ...
    self._fk_data_bridge = WarpBridge(self._wp_fk_data)
  ...
  def create_graph(self) -> None:
    ...
    if self.use_cuda_graph:
      with wp.ScopedDevice(self.wp_device):
        ...
        with wp.ScopedCapture() as capture:
          mjwarp.forward(self.wp_model, self.wp_fk_data)
        self.fk_forward_graph = capture.graph
  ...
  @property
  def wp_fk_data(self) -> mjwarp.Data:
    return self._wp_fk_data
  ...
  @property
  def fk_data(self) -> "DataBridge":
    return cast("DataBridge", self._fk_data_bridge)
  ...
  def fk_forward(self) -> None:
    with wp.ScopedDevice(self.wp_device):
      if self.use_cuda_graph and self.fk_forward_graph is not None:
        wp.capture_launch(self.fk_forward_graph)
      else:
        mjwarp.forward(self.wp_model, self.wp_fk_data)
